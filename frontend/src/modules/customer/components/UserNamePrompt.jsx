...
import { User, ArrowRight, Loader, CheckCircle, Bell, Sparkles, AlertCircle, PartyPopper } from 'lucide-react';
...
const UsernamePage = () => {
  ...
  // ============================================
  // CHECK IF RETURNING USER (on username change)
  // ============================================
  useEffect(() => {
    const checkReturningUser = async () => {
      ...
      try {
        ...
        const response = await axios.get(`/customers/check/${encodeURIComponent(username.trim())}`);
        const exists = response?.data?.exists || response?.exists || false;
        setIsReturningUser(exists);
        
        if (exists) {
          console.log('👋 Returning user detected:', username);
        }
      } catch (err) {
        // If endpoint doesn't exist or fails, assume new user
        setIsReturningUser(false);
      }
    };

    // Debounce the check
    const timeoutId = setTimeout(checkReturningUser, 500);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('📋 Data:', { name, tableId, tableNumber, method, isReturningUser });
      ...
      const customerResponse = await axios.post('/customers', {
        username: name,        // ✅ FIXED: Changed from 'name' to 'username'
        name: name,            // ✅ Also send as 'name' for display
        tableNumber,
        tableId,
        method,
      });
      ...
      try {
        const notificationResponse = await axios.post('/requests/customer-arrived', {
          ...
          customerName: name,
          sessionId: session._id,
          message: `${name} has arrived at Table ${tableNumber}${isReturningUser ? ' (Returning customer)' : ''}`,
          type: 'customer_arrival',
          isReturningCustomer: isReturningUser,
        });
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-text-primary mb-3 leading-tight">
            <span className="block">
              {isReturningUser ? 'Welcome Back!' : 'Almost There!'}
            </span>
            <span className="bg-gradient-to-r from-brand to-secondary bg-clip-text text-transparent">
              {isReturningUser ? "Great to See You Again!" : "What's Your Name?"}
            </span>
          </h1>
          ...
          <div className="bg-bg-elevated rounded-2xl p-6 border-2 border-brand/20 shadow-xl">
            ...
            <label className="block mb-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={loading}
                />
                {isReturningUser && username.trim().length >= 2 && (
            ...
            >
              {loading ? (
                ...
                <>
                  {isReturningUser ? 'Continue' : 'Start Ordering'}
        ...
        <div className="mt-6 text-center bg-bg-elevated/60 backdrop-blur-sm rounded-xl p-4 border border-border">
          <p className="text-sm text-text-secondary">
            <span className="font-bold text-brand">
              {isReturningUser ? 'Glad you\'re back:' : 'Why we ask:'}
            </span>{' '}
            {isReturningUser 
              ? 'We saved your preferences for a better experience! 🎯'
              : 'Your name helps our staff personalize your service! 🌟'}
          </p>
        </div>
      </div>
