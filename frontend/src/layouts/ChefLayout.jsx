// frontend/src/layouts/ChefLayout.jsx
...
/**
 * Chef Layout
 * Layout wrapper for chef/kitchen dashboard
 * Includes sidebar navigation and kitchen-specific features
 */
const ChefLayout = () => {
  ...
  const navigationItems = [
    ...
    { name: 'Kitchen Display', path: '/chef/kitchen-display', icon: ChefHat },
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 bg-gradient-to-b from-red-900 to-red-800 text-white transform transition-transform duration-300 ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-red-700">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-300">Kitchen Panel</p>
            </div>
          </div>
          <button
            onClick={() => setIsSidebarOpen(false)}
            className="lg:hidden text-red-300 hover:text-white"
          >
            <X size={20} />
          </button>
        </div>
        ...
        {/* Bottom User Info */}
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-red-700">
          <div className="flex items-center space-x-3 px-4 py-3 bg-red-700 rounded-lg">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-white">{user?.name || 'Chef'}</p>
              <p className="text-xs text-red-300">Chef</p>
            </div>
          </div>
        </div>
      </aside>
